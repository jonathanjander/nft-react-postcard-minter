//from https://github.com/neha01/nft-demo/tree/master/scripts

// const fs = require('browserify-fs').promises;
const fs = require('fs').promises;
const path = require('path');
const ipfsHash = require('ipfs-only-hash');

// https://stackoverflow.com/questions/36856232/write-add-data-in-json-file-using-node-js
// Accepts json data and stores in specified filePath.
// If the file does not exists in specified location, it creates it
const storeDataToFile = async (jsonData) => {
    try {
        const filePath = path.join(__dirname, "../public/data/ipfsHash.json");
        const ipfsFileExists = await fileExists(filePath);
        if (!ipfsFileExists) {
            console.log('ipfsFileExists: ', ipfsFileExists);
            // First time creating an empty file with [].
            // We will be storing all ipfsHashes as array of objects
            await fs.writeFile(filePath, JSON.stringify([]));
        }
        const data = await fs.readFile(filePath, 'utf8');
        const json = JSON.parse(data);
        json.push(jsonData);
        await fs.writeFile(filePath, JSON.stringify(json,null,2));
    } catch (err) {
        console.log('Error occured while storing data to file', err);
    }
};
const createMetadataFile = async (name, desc, pathOfImageFile) => {
    try{
        const filePath = path.join(__dirname, '../public/data/');
        const hash = await getIpfsHash(pathOfImageFile);
        //const metadataExists = await fileExists(filePath);
        let metadata = {
            name: name,
            description: desc,
            image: 'https://ipfs.io/ipfs/'+ hash
        };
        let data = JSON.stringify(metadata,null,2);
        await fs.writeFile(filePath+name+'.json', data);
    }
    catch (err) {
        console.log('Error occured while storing data to file', err);
    }
}
const testMetadata = async (name, desc, pathOfImageFile) => {
    try{
        console.log(name + "  " + desc + "img");
        const filePath = path.join(__dirname, '../data/');
        let metadata = {
            name: name,
            description: desc,
            image: 'https://ipfs.io/ipfs/'+ pathOfImageFile
        };
        let data = JSON.stringify(metadata,null,2);
        await fs.writeFile(filePath+name+'.json', data);
    }
    catch (err) {
        console.log('Error occured while storing data to file', err);
    }
}
//returns hash that is also being used for ipfs (sha256)
async function getIpfsHash(path){
    try{
        const ipfsFileExists = await fileExists(path);
        if(!ipfsFileExists){
            // throwError("file doenst exist")
            throw new Error('file doenst exist!')
        }
        const data = await fs.readFile(path, 'utf8');
        const hash = await ipfsHash.of(data)
        console.log("hash: "+hash);
        return hash;
    }
    catch (err) {
        console.log('Error occured while getting path of file', err);
    }

}

async function fileExists(path) {
    try {
        const res = await fs.access(path);
        return true;
    } catch (err) {
        // no such file or directory. File really does not exist
        if (err.code == 'ENOENT') {
            return false;
        }
        console.log('Exception fs.statSync (' + path + '): ' + err);
        // some other exception occurred
        throw err;
    }
}

module.exports = {
    storeDataToFile,
    fileExists,
    createMetadataFile,
    testMetadata
};
